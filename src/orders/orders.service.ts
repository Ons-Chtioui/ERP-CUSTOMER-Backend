  ...
  private async deductStock(order: Order, _userId: number): Promise<void> {
    await this.dataSource.transaction(async (manager) => {
      for (const line of order.lines) {
        ...
        if (fromAssembly > 0) {
          const bomCount = await manager.count(BomLine, { where: { product: { id: line.productId } } });
          if (bomCount === 0) {
            throw new BadRequestException(
  ...
  private async restoreStock(order: Order): Promise<void> {
    await this.dataSource.transaction(async (manager) => {
      for (const line of order.lines) {
        ...
        // Commandes confirmées avant le suivi mixte : tout était en stock fini
