...
let OrdersService = class OrdersService {
    ...
    async getLineFulfillmentByWarehouse(productId, quantity, warehouseId) {
        ...
        if (bomLines.length > 0) {
            const assemblablePerBom = await Promise.all(bomLines.map(async (bom) => {
    ...
    async getFinishedStockInWarehouse(productId, warehouseId, manager) {
    ...
    async deductFinishedStock(manager, productId, quantity, warehouseId) {
        ...
        if (remaining > 0)
            throw new common_1.BadRequestException(`Stock fini insuffisant dans cet entrepôt (productId=${productId})`);
    }
    async restoreFinishedStock(manager, productId, quantity, warehouseId) {
        if (quantity <= 0)
            return;
    ...
    async deductComponents(manager, productId, quantity, warehouseId, productName) {
        ...
        for (const bom of bomLines) {
            ...
            if (remaining > 0)
                throw new common_1.BadRequestException(`Stock composant "${bom.component.nom}" insuffisant dans cet entrepôt`);
        }
    }
    async restoreComponents(manager, productId, quantity, warehouseId) {
        if (quantity <= 0)
            return;
        const bomLines = await manager.find(bom_line_entity_1.BomLine, {
            where: { product: { id: productId } },
    ...
    async checkStock(order) {
        ...
        for (const line of order.lines) {
            const fulfillment = await this.getLineFulfillmentByWarehouse(line.productId, line.quantity, order.warehouseId);
    ...
    async deductStock(order, _userId) {
        await this.dataSource.transaction(async (manager) => {
            for (const line of order.lines) {
                const stockFini = await this.getFinishedStockInWarehouse(line.productId, order.warehouseId, manager);
                const bomLines = await manager.find(bom_line_entity_1.BomLine, {
                ...
                for (const bom of bomLines) {
                    const compRaw = await manager
                        ...
                        .getRawOne();
                    stockFabricable = Math.min(stockFabricable, Math.floor(Number(compRaw?.total ?? 0) / Number(bom.quantity)));
                }
                if (!isFinite(stockFabricable))
                    stockFabricable = 0;
                ...
                if (fromAssembly > stockFabricable)
                    throw new common_1.BadRequestException(`Stock insuffisant dans l'entrepôt sélectionné pour ${line.product?.nom}`);
                if (fromStock > 0)
                ...
                for (const supp of line.supplements ?? []) {
                    ...
                    if (remaining > 0)
                        throw new common_1.BadRequestException(`Stock insuffisant pour le supplément "${supp.component?.nom}" dans l'entrepôt sélectionné`);
    ...
    async restoreStock(order) {
        await this.dataSource.transaction(async (manager) => {
            for (const line of order.lines) {
                ...
                await manager.update(order_line_entity_1.OrderLine, line.id, { qtyFromStock: 0, qtyFromAssembly: 0 });
                for (const supp of line.supplements ?? []) {
    ...
    async recordHistory(orderId, fromStatus, toStatus, changedBy, comment) {
        await this.historyRepo.save(this.historyRepo.create({ orderId, fromStatus, toStatus, changedBy, comment: comment ?? null }));
    }
};
